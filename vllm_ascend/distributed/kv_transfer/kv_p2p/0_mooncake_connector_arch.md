...
> 📖 **阅读指南**：本文档按照「入门 → 基础 → 进阶 → 高级 → 专家」五个层次组织。从上游 vLLM 的 Mooncake 两大体系讲起，深入到 vllm-ascend 的特有优化。小白请从第1章开始按顺序阅读；有经验的同学可以直接跳转到感兴趣的章节。
...
- [3. Mooncake 是什么？（月饼传输引擎的两种模式）](#3-mooncake-是什么月饼传输引擎的两种模式)

### 🔵 基础篇（架构入门）
- [4. 整体架构全景图（上游 vLLM + vllm-ascend）](#4-整体架构全景图上游-vllm--vllm-ascend)
...
### 🟣 上游 vLLM 专题
- [10. 上游 MooncakeConnector（P2P 直传）详解](#10-上游-mooncakeconnectorp2p-直传详解)
- [11. 上游 MooncakeStoreConnector（分布式共享存储）详解](#11-上游-mooncakestoreconnector分布式共享存储详解)
- [12. 上游 Store 子系统深度解析](#12-上游-store-子系统深度解析)

### 🔴 高级篇（Ascend 特有）
- [13. Ascend 特有优化详解](#13-ascend-特有优化详解)
- [14. HMA 混合内存分配器支持](#14-hma-混合内存分配器支持)
- [15. Mamba / MLA 特殊支持](#15-mamba--mla-特殊支持)

### ⚫ 专家篇（对比与调优）
- [16. 与上游 vLLM Mooncake 的详细对比](#16-与上游-vllm-mooncake-的详细对比)
- [17. 附录：代码索引与环境变量](#17-附录代码索引与环境变量)
...
| **实现复杂度** | 较低 | 较高 |
| **适用场景** | 推测解码（Speculative Decoding） | 前缀缓存、多实例共享 KV |
...
同时也会详细讲解**上游 vLLM 的 Store 模式**，让你全面了解 Mooncake 生态。

```
本文档结构：
  入门篇 → 基础篇 → 进阶篇 → 【上游 vLLM 专题】→ 高级篇（Ascend特有）→ 专家篇
```
        ...
        └── kv_transfer/
            └── kv_connector/
                └── v1/
                    ...
                    │
                    └── mooncake/
                        ├── mooncake_connector.py  ← 【模式一】P2P 直传
...
└── vllm_ascend/
    └── distributed/
        └── kv_transfer/
            ├── kv_p2p/
            │   └── mooncake_connector.py    ← 【主角】Ascend 版 P2P 连接器
            └── utils/
                ├── mooncake_transfer_engine.py  ← 传输引擎单例（GlobalTE）
                └── utils.py                      ← 工具函数（对齐、合并等）
```

> 💡 **小白笔记**：上游 vLLM 有两个 connector：`MooncakeConnector`（P2P）和 `MooncakeStoreConnector`（Store）。vllm-ascend 主要在 P2P 模式上做了深度优化，所以本文档以 P2P 为主线。
...
## 10. 上游 MooncakeConnector（P2P 直传）详解

> 💡 **阅读提示**：本章讲上游 vLLM 的 P2P 模式 MooncakeConnector。vllm-ascend 的版本就是在这个基础上扩展优化的。对比着学，理解更深刻。
...
#### 10.2.2 MooncakeXferMetadata —— 传输元数据
...
#### 10.2.3 PullReqMeta / SendBlockMeta
...
### 10.4 上游特有的：Bootstrap Server（引导服务器）
...
### 10.5 上游 P2P 模式 vs vllm-ascend P2P 模式（快速对比）
...
## 11. 上游 MooncakeStoreConnector（分布式共享存储）详解

> ⚠️ **重要**：这是上游 vLLM 独有的功能！vllm-ascend 目前没有 Store 模式。
> 
> 虽然 vllm-ascend 没有，但理解 Store 模式能帮你更全面地理解 Mooncake 生态。
...
2. 用户 2 请求到达 → 查 Store → 发现前缀已经有了 → 直接取，不用再算！
...
### 12.2 Scheduler 端：MooncakeStoreScheduler
...
#### 12.2.1 核心方法：get_num_new_matched_tokens()
...
### 12.3 Worker 端：MooncakeStoreWorker
...
### 12.4 Coordinator：缓存命中协调器

`MooncakeStoreCoordinator` 是用来计算「最长缓存命中长度」的。

#### 12.4.1 为什么需要 Coordinator？
...
#### 12.4.2 核心方法：find_longest_cache_hit()
...
### 14.2 MooncakeConnector 怎么支持 HMA？
